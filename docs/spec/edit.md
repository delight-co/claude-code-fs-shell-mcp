...
- **Read-equivalent shell commands.** The upstream tool counts certain single-file shell invocations as Read-equivalents for satisfying the read-before-edit precondition. The recognised set is the same as for [`Write`](./write.md#known-gaps) (`cat`, `nl`, `bat`, `batcat`, `sed -n 'Np'` / `sed -n 'N,Mp'`, `head`, `tail`, `grep` / `egrep` / `fgrep`, `rg`, with no pipes or redirects; `grep` / `rg` additionally require an exit-zero result). The MCP server will only honour these once it implements the bash tool; the rules will then live in [`bash.md`](./bash.md). Until then, only an explicit `Read` call seeds the read-tracking state.
- **Smart-quote normalisation table.** The exact set of curly-quote codepoints recognised by the upstream tool and the position-dependent rules used to pick left vs right quotes during `new_string` synchronisation are tested as part of implementation; this spec captures the dominant pairs (`U+2018`/`U+2019` for single, `U+201C`/`U+201D` for double).
